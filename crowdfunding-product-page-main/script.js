...
const dasboardTotalBackers = dashboard.querySelector(
  ".backers > p:first-of-type",
);
const dashboardDays = dashboard.querySelector(".days > p:first-of-type");
const dashboardProgress = dashboard.querySelector("progress");

const selectRewardBtns = main.querySelectorAll(
  "main > .about > .card > .select-reward",
);
...
function disableOthers(currentElement) {
  UIElements.filter((element) => element !== currentElement).forEach(
    (element) => {
...
function updateDasboard() {
  ...
  dashboardSum.textContent = data.currentSum.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  });
...
selectRewardBtns.forEach((btn) =>
  btn.addEventListener("click", (event) => {
    ...
    const selectedProduct = basketElement.querySelector(
      `.card[data-product=${event.target.dataset.product}]`,
    );
    // selectedProduct.style.backgroundColor = 'red';
    const productRadioBtn = selectedProduct.querySelector(
      'input[type="radio"]',
    );
...
function updateQtyFields() {
  ...
  Object.entries(data.stock).forEach(([product, qty]) => {
    const productQtyFields = Array.from(qtyFields).filter(
      (field) => field.dataset.product === product,
    );
